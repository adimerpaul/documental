...
<!--        </template>-->
        <template v-slot:body-cell-opcion="props">
          <q-td key="opcion" :props="props">
            <q-btn-group>
<!--              <q-btn v-if="props.row.estado!='SOLICITADO'" dense round flat color="positive" @click="solicitar(props.row.id)" icon="check_circle" label="devolver"></q-btn>-->
              <q-btn class="q-pa-xs" dense round flat color="negative"  @click="cancelar(props.row)" icon="close" label="cancelar"></q-btn>
              <q-btn class="q-pa-xs" dense round flat color="positive"  @click="aceptar(props.row)" icon="done" label="aceptar"></q-btn>
            </q-btn-group>
          </q-td>
        </template>
...
export default {
  ...
  methods:{
    ...
    aceptar(d){
      ...
      // d.estado='ACEPTADO'
      // d.observacion=''
      this.$axios.put(process.env.API+'/prestamo/'+d.id,{estado:'ACEPTADO'}).then(res=>{
    ...
    misdatos(){
      this.$q.loading.show();
      this.$axios.get(process.env.API+'/prestamo/1').then(res=>{
        // console.log(res.data)
        this.data=res.data;
